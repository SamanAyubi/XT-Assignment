...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.femaleFilterSub$ = this.femaleCheckbox.valueChanges.pipe(
      startWith("")
    );
    this.humanFilterSub$ = this.humanCheckbox.valueChanges.pipe(startWith(""));
    this.mythologyFilterSub$ = this.mythologyCheckbox.valueChanges.pipe(
      startWith("")
    );
    this.otherSpeciesFilterSub$ = this.otherSpeciesCheckbox.valueChanges.pipe(
      startWith("")
    );
    this.earthFilterSub$ = this.earthCheckbox.valueChanges.pipe(startWith(""));
    this.nuptiaFilterSub$ = this.nuptiaCheckbox.valueChanges.pipe(
      startWith("")
    );
    this.unknownFilterSub$ = this.unknownCheckbox.valueChanges.pipe(
      startWith("")
    );
    this.otherOriginsFilterSub$ = this.otherOriginsCheckbox.valueChanges.pipe(
      startWith("")
    );
    ...
    this.filteredStates$ = combineLatest(
      ...
      this.nameFilterSub$,
      this.maleFilterSub$,
      this.femaleFilterSub$,
      this.humanFilterSub$,
      this.mythologyFilterSub$,
      this.otherSpeciesFilterSub$,
      this.earthFilterSub$,
      this.nuptiaFilterSub$,
      this.unknownFilterSub$,
      this.otherOriginsFilterSub$,
      this.orderSortSub$
    ).pipe(
      map(([apiData, filterString]) => {
        ...
        uiData = apiData.results.filter((results) =>
          results.name.toLowerCase().includes(filterString)
        );

        formEle = document.getElementById("male") as HTMLInputElement;
        if (formEle.checked) {
          uiData = uiData.filter(
            (results) =>
              results.gender.trim().toLowerCase().localeCompare("male") === 0
          );
        }
        ...
        if (formEle.checked) {
          uiData = uiData.filter(
            (results) =>
              results.gender.trim().toLowerCase().localeCompare("female") === 0
          );
        }
        ...
        if (formEle.checked) {
          uiData = uiData.filter(
            (results) =>
              results.species.trim().toLowerCase().localeCompare("human") === 0
          );
        }
        ...
        if (formEle.checked) {
          uiData = uiData.filter((results) =>
            results.origin.name.trim().toLowerCase().includes("earth")
          );
        }
